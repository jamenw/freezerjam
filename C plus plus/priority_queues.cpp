...
void priorityQueue<T>::push_back(const T& value) {
	if (front == back && priorityValues[front] != 0) { //if it's full, throw an error 
		throw 1;
	}
	else {
		values[back] = value; //insert at the back the value passed in 
		priorityValues[back] = 16; //set default value of 16 at the index 
		back++; //move back one so you are at next available index 
		if (back == capacity) { //allows for looping 
			back = 0;
		}
	}
	//push_back(value, 16); //this line can replace all of what I have in this function 
}

template <typename T>
void priorityQueue<T>::push_back(const T& value, int priority) { //same as above except for one line, I realized I could have combined the two  
	if (front == back && priorityValues[front] != 0) {            //functions by using what I put on line 117 
		throw 1;
	}
	else {
		values[back] = value;
...
T priorityQueue<T>::pop_front() {
	if (front == back && priorityValues[front] == 0) { //if empty, throw the error 
		...
		int index = front; //keep track of where the front was so we can reset it 
		do {
			front++; //move front along so we can compare 
			if (front == capacity) { //loop around list if we have to 
				front = 0;
			} if (front == back && priorityValues[index] != 0) { //this allowed me to set the last nonzero value to zero once first == back, it wouldn't get that last zero without it 
				priorityValues[index] = 0;
				return values[index];
			}
		} while (priorityValues[front] == 0); //keep moving along until you found a nonzero 
		priorityValues[index] = 0; //we set the priority to zero 
		return values[index]; //we return that value 
	}
}
...
T priorityQueue<T>::pop_highest_priority() {
	...
	bool ok = true; //I can't remember why I needed this bool, but I think it was because I needed a do while because my scan was always equaling my back variable 
	int biggest = front; //store our index 
	int scan = front; //store our index again 
	while (scan != back || ok) {
		ok = false; //insures that as long as scan doesn't equal back, we'll keep looping 
		if (scan == capacity) { //to loop around 
			scan = 0;
		}
		else if (priorityValues[biggest] < priorityValues[scan]) { //finds the first largest value 
			biggest = scan; //we store our index 
			scan++; //keep moving in the list 
		}
		else {
			scan++; //else keep moving in the list 
		}
	} //we now have our biggest priority index 
	if (biggest == front) { //at the front, just call pop_front() 
		pop_front();
		return values[biggest];
	}
	else if ((biggest + 1) % capacity == back) { // else if at the back, pretty much just custom pop code 
		back = biggest;
	...
	else { //just pop it at that value 
		priorityValues[biggest] = 0;
		return values[biggest];
	}
}
